...
if ( ! function_exists( 'cb_clean_excerpt' ) ) {  
    function cb_clean_excerpt ($data, $readmore = false) {
...
if ( ! function_exists( 'cb_byline' ) ) {  
    function cb_byline($cb_cat = true, $cb_post_id = NULL, $cb_short_comment_line = false) {
...
if ( ! function_exists( 'cb_review_boxes' ) ) {  
    function cb_review_boxes($post){
        ...
        if ($cb_review_checkbox == 'on') {
                ...
                $cb_pros_title = '<div class="cb-title">'. get_post_meta($cb_post_id, 'cb_pros_title', true ) .'</div>';
                $cb_cons_title = '<div class="cb-title">'. get_post_meta($cb_post_id, 'cb_cons_title', true ) .'</div>';
                $cb_pro_1 = '<li>'. get_post_meta($cb_post_id, 'cb_pro_1', true ) .'</li>';
                $cb_pro_2 = '<li>'. get_post_meta($cb_post_id, 'cb_pro_2', true ) .'</li>';
                $cb_pro_3 = '<li>'. get_post_meta($cb_post_id, 'cb_pro_3', true ) .'</li>';
                $cb_con_1 = '<li>'. get_post_meta($cb_post_id, 'cb_con_1', true ) .'</li>';
                $cb_con_2 = '<li>'. get_post_meta($cb_post_id, 'cb_con_2', true ) .'</li>';
                $cb_con_3 = '<li>'. get_post_meta($cb_post_id, 'cb_con_3', true ) .'</li>';                
...
if ( ! function_exists( 'cb_related_posts' ) ) {        
    function cb_related_posts() {
        ...
        if ( $cb_full_width_post == 'nosidebar' ) { $cb_number_related = 3; } else { $cb_number_related = 2;}
...
if ( $cb_background_color != NULL ) {
    ...
    if ( $cb_background_image != NULL ) { echo '#151515;'; } else { echo $cb_background_color; }
    echo ';}';
}  
...
echo '#cb-main-menu .current-post-ancestor, #cb-main-menu .current-menu-item, #cb-main-menu .current-menu-ancestor, #cb-main-menu .current-post-parent, #cb-main-menu .current-menu-parent, #cb-main-menu .current_page_item, #cb-main-menu .current-page-ancestor, #cb-main-menu .current-category-ancestor, .cb-review-box .cb-bar .cb-overlay span, #cb-accent-color, .cb-highlight { background-color: '. $cb_base_color .';}</style>'; 
...
if ( ! function_exists( 'cb_custom_code' ) ) {   
    function cb_custom_code(){
            ...
            $cb_custom_head = ot_get_option('cb_custom_head', false);
            $cb_custom_css = ot_get_option('cb_custom_css', false);
            $cb_custom_a_css = ot_get_option('cb_link_color', false);
